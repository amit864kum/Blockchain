...
def handle_block(block):
    if block.is_valid():
        ...
        print("\n✅ [Miner2] Received and validated block:")
        block.display()
...
def mine_loop():
    global chain_head
    SocketUtils.start_transaction_server(handle_transaction, 5001)
    SocketUtils.start_block_server(handle_block, 6001)
    ...
    while True:
        ...
        SocketUtils.send_block_to_all(new_block, 6001)

        print("[Miner2] Waiting for votes...")
        while len(validation_pool[block_hash]) < 2:
            time.sleep(0.5)

        print("✅ [Miner2] Block approved.")
