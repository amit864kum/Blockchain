...
class Tx:
    def __init__(self):
        self.inputs = []      # list of tuples: (serialized_public_key_string, amount)
        self.outputs = []     # list of tuples: (serialized_public_key_string, amount)
        self.reqs = []        # list of serialized public keys
        self.sigs = []        # list of digital signatures
    ...
    def is_valid(self):
        ...
        for i, (ser_pubkey, _) in enumerate(self.inputs):
            ...
            if i >= len(self.sigs):
                print("❌ Missing signature for input.")
                return False
            if not Signatures.verify(message, self.sigs[i], pubkey):
                print("❌ Invalid signature for input.")
                return False
        ...
        for ser_req in self.reqs:
            ...
            for sig in self.sigs:
                if Signatures.verify(message, sig, pubkey):
                    ...
                    break
            if not found:
                print("❌ Missing signature for required address.")
                return False
